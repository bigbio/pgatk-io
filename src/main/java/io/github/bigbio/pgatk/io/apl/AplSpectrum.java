package io.github.bigbio.pgatk.io.apl;
...
public class AplSpectrum implements Spectrum {
    ...
    public AplSpectrum(String mgfQuery, Long index) throws PgatkIOException {
        ...
        for (int nLineNumber = 0; nLineNumber < lines.length; nLineNumber++) {
            ...
            if (nLineNumber == 0 && !"peaklist start".equals(line))
                throw new PgatkIOException("MS2 query must start with 'peaklist start'");

            if (nLineNumber == 0) continue;

            if (nLineNumber == lines.length - 1 && !"peaklist end".equals(line))
                throw new PgatkIOException("MS2 query must end with 'peaklist end'");

            if (nLineNumber == lines.length -1) continue;

            // check if it's a property
            Matcher attributeMatcher = AplIndexedReader.attributePattern.matcher(line);

            if (inAttributeSection && attributeMatcher.find()) {
                if (attributeMatcher.groupCount() != 2)
                    throw new PgatkIOException("Invalid attribute line encountered in MS2 query.");
     ...
     */
    private void saveAttribute(String name, String value) throws PgatkIOException {
        if ("header".equals(name))
            ...
            throw new PgatkIOException("Unknown peptide property '" + name + "' encountered");
    }
